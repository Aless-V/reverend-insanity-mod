...
// Killer Move Development Interface: Combine Gu insects to develop custom Killer Moves.
public class DeductionScreen extends Screen {
    ...
    public DeductionScreen() {
        super(Component.literal("Killer Move Development"));
    }
    ...
    private void renderTitle(GuiGraphics g, int px, int py) {
        String title = "K I L L E R M O V E D E D U C T I O N";
    ...
    private void renderGuList(GuiGraphics g, int mouseX, int mouseY, int px, int py) {
        ...
        g.drawString(this.font, "[ Aperture Gu Insects ]", sx, sy, HEADER_COL);
        sy += 14;
        ...
        for (int i = guScrollOffset; i < end; i++) {
            ...
            String rankPath = gu.rank() + "Rank" + gu.pathName();
        ...
        if (guList.isEmpty()) {
            g.drawString(this.font, "Aperture contains no Gu.", sx + 10, sy + listH / 2 - 4, DIM_COL);
        }

        sy += listH + 4;
        g.drawString(this.font, "Left-click = Core | Right-click = Support", sx, sy, DIM_COL);
    }
    ...
    private void renderRecipe(GuiGraphics g, int mouseX, int mouseY, int px, int py) {
        ...
        g.drawString(this.font, "[ Deduction Recipe ]", sx, sy, HEADER_COL);
        sy += 16;

        List<SyncDeductionScreenPayload.DeductionGuEntry> guList = ClientDataCache.getDeductionGuList();

        g.drawString(this.font, "Core Gu Insect:", sx, sy, TEXT_COL);
        if (selectedCoreIndex >= 0 && selectedCoreIndex < guList.size()) {
            ...
            g.drawString(this.font, "(Not selected)", sx + 55, sy, DIM_COL);
        }
        sy += 15;

        for (int slot = 0; slot < 4; slot++) {
            g.drawString(this.font, "Support" + (slot + 1) + ":", sx, sy, TEXT_COL);
            if (slot < selectedSupportIndices.size()) {
                ...
                g.drawString(this.font, "(Empty)", sx + 55, sy, DIM_COL);
            }
            sy += 13;
        }

        sy += 8;
        g.drawString(this.font, "Target Path:", sx, sy, TEXT_COL);
        ...
        if (selectedCoreIndex >= 0) {
            ...
            g.drawString(this.font, "Gu Count: " + guCount, sx, sy, TEXT_COL);
            sy += 13;
            g.drawString(this.font, "Estimated Time: " + String.format("%.0f", seconds) + "seconds", sx, sy, TEXT_COL);
            sy += 13;
            g.drawString(this.font, "Primeval Essence Cost: ~" + (int) essenceCost, sx, sy, 0xFF88FF88);
            sy += 13;
            g.drawString(this.font, "Thought Cost: ~" + (int) thoughtsCost, sx, sy, 0xFF88AAFF);

            if (crossPath) {
                sy += 13;
                g.drawString(this.font, "Cross-Path Deduction (Difficulty +)", sx, sy, 0xFFFF8844);
            }
            ...
            if (!allPaths.isEmpty()) {
                ...
                if (!reactions.isEmpty() || !stacks.isEmpty()) {
                    ...
                    sy += 4;
                    g.drawString(this.font, "[ Dao Resonance ]", sx, sy, TITLE_COL);
                    sy += 12;
            ...
            g.drawString(this.font, "Select a core Gu.", sx, sy, DIM_COL);
        }
    }
    ...
    private void renderDeductionStatus(GuiGraphics g, int px, int py) {
        ...
        if (ClientDataCache.isDeductionActive()) {
            ...
            g.drawString(this.font, "Deduction in progress...", sx, sy, TITLE_COL);
            g.drawString(this.font, String.format("Success Rate: %.0f%%", rate * 100), sx + 100, sy, 0xFF88FF88);
            sy += 14;
    ...
    private void renderDeductionResult(GuiGraphics g, int px, int py) {
        ...
        String label = switch (outcome) {
            case 0 -> "Critical Success!";
            case 1 -> "Success";
            case 2 -> "Partial Success";
            case 3 -> "Failure";
            case 4 -> "New Discovery!";
            default -> "Completed";
        };
        ...
        if (!moveName.isEmpty()) {
            String display = "Killer Move: " + moveName;
    ...
    private void renderButtons(GuiGraphics g, int mouseX, int mouseY, int px, int py) {
        ...
        if (ClientDataCache.isDeductionActive()) {
            renderButton(g, "Cancel Deduction", px + 10, by, 80, mouseX, mouseY);
        } else {
            boolean canStart = selectedCoreIndex >= 0;
            renderButton(g, "Start Deduction", px + 10, by, 80, mouseX, mouseY, canStart);
        }
        renderButton(g, "Clear Selection", px + 100, by, 80, mouseX, mouseY);
    ...
    private void renderPathSelector(GuiGraphics g, int mouseX, int mouseY, int px, int py) {
        ...
        g.drawString(this.font, "Choose Target Path", sx + 10, sy + 5, TITLE_COL);
        sy += 20;
        ...
        renderButton(g, "Confirm", sx + selectorW / 2 - 30, closeY, 60, mouseX, mouseY);
    }
